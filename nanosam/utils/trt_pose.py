...
class PoseDetector(object):
    def __init__(self, weights, config, arch="densenet121_baseline_att", shape=(256, 256)):
        ...
        if arch == "densenet121_baseline_att":
            self.model = (
                trt_pose.models.densenet121_baseline_att(num_parts, 2 * num_links).cuda().eval()
            )
        ...
        for i in range(count):
            ...
            for j in range(C):
                ...
                if k >= 0:
                    ...
                    kp = {"name": self.keypoints[j], "index": j, "x": x, "y": y}
